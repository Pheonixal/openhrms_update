odoo.define('qz_document_expiry.disciplinary_action', function (require) {
"use strict";
...
var DisciplinaryRenderer = AbstractGroupedOne2ManyRenderer.extend({

    groupBy: 'line_type_id',
    groupTitleTemplate: 'hr_resume_group_row',
    dataRowTemplate: 'disciplinary_action_data_row',

    _formatData: function (data) {
        var dateFormat = time.getLangDateFormat();
        try{
            var action = data.action.data.display_name || "";
            }
        catch{
    ...
    _renderBody: function () {
        ...
        for (var key in grouped_by) {
            var group = grouped_by[key];
            if (key === 'undefined') {
                groupTitle = _t("Disciplinary Actions");
            } else {
                groupTitle = group[0].data[self.groupBy].data.display_name;
            }
            var $title_row = $(self._renderGroupRow(groupTitle));
...
var FieldDisciplinary = FieldOne2Many.extend({
    ...
    _getRenderer: function () {
        return DisciplinaryRenderer;
    },
});


field_registry.add('disciplinary_action', FieldDisciplinary);
